...
function AISandboxPage() {
  ...
  const [activeTab, setActiveTab] = useState<"generation" | "preview">(
    "preview",
  );
  ...
  const [hasInitialSubmission, setHasInitialSubmission] =
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      const storedStyle =
        templateParam || sessionStorage.getItem("selectedStyle");
      ...
      try {
        if (sandboxIdParam) {
          ...
          console.log(
            "[home] No sandbox in URL, creating new sandbox automatically...",
          );
          sandboxCreated = true;
          await createSandbox(true);
        }
        ...
        console.error(
          "[ai-sandbox] Failed to create or restore sandbox:",
          error,
        );
        if (isMounted) {
  ...
  useEffect(() => {
    ...
    if (autoStart === "true" && !showHomeScreen && homeUrlInput) {
      ...
      setTimeout(() => {
        console.log(
          "[generation] Auto-starting generation for URL:",
          homeUrlInput,
        );
        startGeneration();
      }, 1000);
  ...
  const checkAndInstallPackages = async () => {
    ...
    addChatMessage(
      "Checking packages... Sandbox is ready with Vite configuration.",
      "system",
    );
  };
  ...
  const checkSandboxStatus = async () => {
    try {
      ...
      if (data.active && data.healthy && data.sandboxData) {
        console.log(
          "[checkSandboxStatus] Setting sandboxData from API:",
          data.sandboxData,
        );
        ...
        if (!sandboxData) {
          console.log(
            "[checkSandboxStatus] No existing sandboxData, clearing state",
          );
          setSandboxData(null);
...
Tip: I automatically detect and install npm packages from your code imports (like react-router-dom, axios, etc.)`,
    ...
    try {
            ...
            try {
              ...
              switch (data.type) {
                ...
                case "command-progress":
                  addChatMessage(
                    `${data.action} command: ${data.command}`,
                    "command",
                    { commandType: "input" },
                  );
                  break;
      ...
      if (finalData && finalData.type === "complete") {
        ...
        if (data.success) {
          ...
          console.log(
            "[applyGeneratedCode] Current iframe src:",
          ...
          if (results.filesCreated?.length > 0) {
            ...
            if (isEdit) {
              ...
              if (isPartOfGeneration) {
                addChatMessage(
                  `Applied ${results.filesCreated.length} files successfully!`,
                  "system",
                );
              } else {
                addChatMessage(
            ...
            console.log(
              "[build-test] Skipping build test - would need API endpoint",
            );
            ...
            setTimeout(() => {
              ...
              if (iframeRef.current && currentSandboxData?.url) {
                console.log(
                  "[home] Refreshing iframe after code application...",
                );
                ...
                setTimeout(() => {
                  try {
                    ...
                    console.log(
                      "[home] Could not reload iframe (cross-origin):",
                      e,
                    );
                  }
                  // Reload completed
          ...
          if (iframeRef.current && currentSandboxData?.url) {
            ...
            setTimeout(async () => {
              if (iframeRef.current && currentSandboxData?.url) {
                console.log(
                  "[applyGeneratedCode] Starting iframe refresh sequence...",
                );
                console.log(
                  "[applyGeneratedCode] Current iframe src:",
                  iframeRef.current.src,
                ...
                console.log(
                  "[applyGeneratedCode] Sandbox URL:",
                  currentSandboxData.url,
                );
                ...
                try {
                  ...
                  try {
                    ...
                    if (iframeDoc && iframeDoc.readyState === "complete") {
                      console.log(
                        "[applyGeneratedCode] Iframe loaded successfully",
                      );
                      return;
                    }
                  } catch {
                  ...
                  console.error(
                    "[applyGeneratedCode] Direct navigation failed:",
                    e,
                  );
                }
                ...
                console.log(
                  "[applyGeneratedCode] Falling back to iframe recreation...",
                );
                ...
                console.log(
                  "[applyGeneratedCode] Iframe recreated with new content",
                );
              } else {
                console.error(
    ...
    if (
                            ...
                            <span
                              className={`px-2 py-0.5 text-xs rounded ${
                                generationProgress.currentFile.type === "css"
                                  ...
                                  : generationProgress.currentFile.type ===
                                      "javascript"
                          ...
                          <div className="bg-gray-900 border border-gray-700 rounded">
                            ...
                            >
                              {(() => {
                                ...
                                const lastFileEnd =
                                  generationProgress.files.length > 0
                                    ? generationProgress.streamedCode.lastIndexOf(
                                        "</file>",
                                      ) + 7
                                    : 0;
      ...
      const isInitialGeneration =
        ...
        (urlScreenshot ||
          ...
          isPreparingDesign ||
          loadingStage);
      const isNewGenerationWithSandbox =
        isStartingNewGeneration && sandboxData?.url;
                            ...
                            {codeApplicationState.packages.map((pkg, index) => (
                              <span
                                ...
                                className={`px-2 py-1 text-xs rounded-full transition-all ${
                                  codeApplicationState.installedPackages?.includes(
                                    pkg,
                                  )
                    ...
                    {codeApplicationState.stage === "applying" &&
                      codeApplicationState.filesGenerated && (
                        <div className="text-sm text-gray-600">
                          Creating {codeApplicationState.filesGenerated.length}{" "}
                          files...
                        </div>
                      )}
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      const fullContext = {
        sandboxId:
          sandboxData?.sandboxId || (sandboxCreating ? "pending" : null),
            ...
            if (line.startsWith("data: ")) {
              try {
                ...
                if (data.type === "status") {
                  ...
                  setGenerationProgress((prev) => {
                    ...
                    while ((match = fileRegex.exec(newStreamedCode)) !== null) {
                      ...
                      if (!processedFiles.has(filePath)) {
                        ...
                        const existingFileIndex = updatedState.files.findIndex(
                          (f) => f.path === filePath,
                        );
                  ...
                  if (
                    data.packagesToInstall &&
                    data.packagesToInstall.length > 0
                  ) {
                    console.log(
                      "[generate-code] Packages to install from tools:",
                      data.packagesToInstall,
                    );
                  ...
                  while (
                    (fileMatch = fileRegex.exec(data.generatedCode)) !== null
                  ) {
      ...
      if (generatedCode) {
        ...
        if (isEdit && generatedFiles.length > 0) {
          ...
          const editedFileNames = generatedFiles
            .map((f) => f.split("/").pop())
            .join(", ");
        ...
        if (sandboxPromise) {
          ...
          try {
            ...
            setChatMessages((prev) =>
              prev.filter(
                (msg) => msg.content !== "Waiting for sandbox to be ready...",
              ),
            );
          } catch {
            addChatMessage(
  ...
  const downloadZip = async () => {
    if (!sandboxData) {
      addChatMessage(
        "Please wait for the sandbox to be created before downloading.",
        "system",
      );
      return;
    }
  ...
  const startGeneration = async () => {
    ...
    const brandExtensionMode =
      sessionStorage.getItem("brandExtensionMode") === "true";
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const brandExtensionMode =
          sessionStorage.getItem("brandExtensionMode") === "true";
        const brandExtensionPrompt =
          sessionStorage.getItem("brandExtensionPrompt") || "";
        ...
        if (brandExtensionMode) {
          ...
          addChatMessage(
            `Acquired branding format from ${cleanUrl}`,
            "system",
            {
...
Focus on the key sections and content, making it clean and modern.`;
        ...
        if (generatedCode) {
          ...
          addChatMessage(
            ...
            {
              scrapedUrl: url,
              scrapedContent: brandExtensionMode
                ? { brandGuidelines }
                : scrapeData,
              generatedCode: generatedCode,
            },
                ...
                <div className="flex flex-col gap-4">
                  {conversationContext.scrapedWebsites.map((site, idx) => {
                    ...
                    const screenshot =
                      ...
                      sessionStorage.getItem("websiteScreenshot");
                        ...
                        {screenshot && (
                          <div className="w-full">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <button
                                onClick={() =>
                                  setScreenshotCollapsed(!screenshotCollapsed)
                                }
                            ...
                            <div
                              ...
                              style={{
                                ...
                                transform: screenshotCollapsed
                                  ? "translateY(-20px)"
                                  : "translateY(0)",
                                pointerEvents: screenshotCollapsed
                                  ? "none"
                                  : "auto",
                    ...
                    >
                      <div className="block">
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="mt-2 text-xs opacity-70">
                                  Press 'F' or click the Fix button above to
                                  resolve
                                </div>
                              </div>
                            </div>
                        ...
                        {msg.metadata?.brandingData && (
                          <div className="mt-3 bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-xl overflow-hidden max-w-[500px] shadow-sm">
                            ...
                            <div className="p-16">
                              ...
                              {/* Button Styles */}
                              {msg.metadata.brandingData.components
                                ?.buttonPrimary && (
                                <div className="mb-16">
                                  <div className="text-sm font-semibold text-gray-900 mb-8">
                                    Button Styles
                                  ...
                                  <div className="flex flex-wrap gap-12">
                                    <div>
                                      <div className="text-xs text-gray-600 mb-6 font-medium">
                                        Primary Button
                                      </div>
                                      <button
                              ...
                              {msg.metadata.brandingData.personality && (
                                <div className="text-sm">
                                  ...
                                  <span className="font-semibold text-gray-900 capitalize">
                                    {msg.metadata.brandingData.personality.tone}{" "}
                                    tone,{" "}
                                    {
                                      msg.metadata.brandingData.personality
                                        .energy
                        ...
                        {msg.metadata?.appliedFiles &&
                          msg.metadata.appliedFiles.length > 0 && (
                  ...
                  <div className="flex flex-wrap items-start gap-1">
                    ...
                    {generationProgress.currentFile && (
                      <div
                        ...
                        style={{
                          animationDelay: `${generationProgress.files.length * 30}ms`,
                        }}
                      >
                        <div className="w-16 h-16 border-2 border-white border-t-transparent rounded-full animate-spin" />
